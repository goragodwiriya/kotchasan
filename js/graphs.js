  ...
  GGraphs.prototype = {
    initialize: function(id, o) {
      ...
      if (options.max) {
        ...
        this.max += (this.max / options.rows);
    ...
    drawLine: function() {
      ...
      var t = Math.ceil(this.fontSize / 2),
        ...
        b = this.height - this.fontSize - options.labelOffset,
        rows = options.rows,
    ...
    drawPie: function() {
      ...
      var options = this.options,
        ...
        radius = centerY - options.centerOffset,
        counter = 0.0,
      ...
      function drawSlice(slice, index) {
        if (slice.percentage) {
          ...
          context.fillText(toCurrency(slice.value, null, true), labelX, labelY);
        }
    ...
    drawHChart: function() {
      ...
      var options = this.options,
        self = this,
        context = this.context,
        offsetRight = Math.ceil(context.measureText(this.max).width / 2),
        l = 0;
    ...
    drawVChart: function() {
      ...
      b = cellHeight * rows + t;
      var clientHeight = b - t,
        barWidth = Math.max(2, (cellWidth - 8 - 2 * (this.datas.rows.length + 1)) / this.datas.rows.length),
        offsetWidth = l + 6;
      forEach(self.datas.rows, function() {
        forEach(this.items, function(item, index) {
